...
TYPE_ENUM = {"source-note", "book-note", "atom", "molecule", "compound", "moc", "author"}
...
REQUIRED_FIELDS = {
    "source-note": ["type", "source-type", "tags"],
    "book-note":   ["type", "tags", "derived-from"],
    "atom":        ["type", "tags"],
    "molecule":    ["type", "tags"],
    "compound":    ["type", "compound-type", "tags"],
    "moc":         ["type", "tags"],
    "author":      ["type", "tags", "aliases"],
}
...
def validate_note(
    ...
    if not note_type:
        ...
        return issues
    if note_type not in TYPE_ENUM:
        err(f"Unknown type '{note_type}' — not in closed enum {sorted(TYPE_ENUM)}")
        return issues

...
├── ideas/            # synthesis notes about a source: paper reads, book notes-in-brief, analysis
├── compounds/        # original essays, theses, blog posts, projects
...
**v3.0 closed enum** (16 types, 14 canonical + moc + author):
`atom`, `molecule`, `compound`, `academic-paper`, `book`, `podcast-transcript`, `podcast-notes`, `book-notes`, `book-review`, `project`, `open-question`, `idea`, `blog-post`, `journal`, `moc`, `author`

**Deprecated (warn, not error):** `source-note` → use `academic-paper`, `book`, `podcast-transcript`, or `podcast-notes`. `book-note` → use `book-notes`.
...
|---|---|---|
| `academic-paper` | `type`, `tags` | sources/ |
| `book` | `type`, `tags` | sources/ |
| `podcast-transcript` | `type`, `tags` | sources/ |
| `podcast-notes` | `type`, `tags` | sources/ |
| `atom` | `type`, `tags` | atoms/ |
| `molecule` | `type`, `tags` | molecules/ |
| `compound` | `type`, `tags` | compounds/ |
| `book-notes` | `type`, `tags` | book-notes/ |
| `book-review` | `type`, `tags` | compounds/ |
| `project` | `type`, `tags` | compounds/ |
| `open-question` | `type`, `tags` | compounds/ |
| `idea` | `type`, `tags` | ideas/ |
| `blog-post` | `type`, `tags` | compounds/ |
| `journal` | `type`, `tags` | personal/ |
| `moc` | `type`, `tags` | MOCs/ |
| `author` | `type`, `tags` | authors/ |

Recommended (warn if missing): `derived-from` on atom, molecule, compound, book-notes, book-review. `source-type` on academic-paper, book, podcast-transcript, podcast-notes.
Optional on any source type: `source-file`, `url`.

### Filename conventions

All files: kebab-case, no spaces, no capitals. Sources include a year prefix; atoms and molecules do not.

| Type | Convention | Example |
|---|---|---|
| `academic-paper` | `YYYY-title-slug.md` | `2011-hallmarks-of-cancer.md` |
| `book` | `YYYY-firstname-lastname-book-title.md` | `1962-carl-jung-memories-dreams-reflections.md` |
| `podcast-transcript` | `YYYY-podcast-name-episode-slug.md` | `2023-invest-like-the-best-howard-marks.md` |
| `podcast-notes` | `YYYY-podcast-name-episode-slug-notes.md` | `2023-invest-like-the-best-howard-marks-notes.md` |
| `book-notes` | `book-slug-chapter-n-chapter-name.md` | `atlas-shrugged-chapter-1-theme.md` |
...
- Tags come from `_system/tags.md` (the single canonical list). Max 5 per note, kebab-case. Before using a tag confirm it exists there; if not, add it to the Draft section first. `validate.py` warns on unknown tags. New tags are not blocked, add to Draft, then use.

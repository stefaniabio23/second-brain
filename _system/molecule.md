...
7. When the user is ready: read `_templates/molecule.md`. Run `/stop-slop` and `/writing-voice` on the user's body text before writing. If either is not installed: `[skill] not found. Proceed without the check? (y/n)` — continue only on confirmation. Then offer to write the frontmatter and filename to `molecules/[filename].md`.
